...
export const Footer = () => {
  return (
    <footer className="py-8 px-8">
      <img className="w-[280px] pb-2" src="/images/Logo.png" alt="Logo" />

      <div className="xl:flex xl:gap-24 mt-[-25px]">
        ...
        <div className="flex flex-col gap-8 mt-16 md:flex-row md:gap-24 xl:mt-0">
          ...
          <div className="flex justify-center gap-4">
            {medias.map((media) => (
              <div
                key={media.text}
                className="hover:text-Teal-200 cursor-pointer border rounded-full size-[30px] p-0.5"
              >
                {media.icon}
              </div>
            ))}
